...
if msg.type == "syncroChoix" then
        ...
        if msg.type == "unselect" then
            return
        end
        for k,c in pairs(clients) do    
            if c.cookie == client.cookie then
            elseif c.perso == msg.pck.perso then
                -- Le perso est déja reservé
                client:send({type= "error", pck={current="ChoixPerso", content ="Personnage déjà selectionné"}})
                return
            end
        end
        assert(msg.pck.perso ~=nil)
        client.perso = msg.pck.perso 

        for k,c in pairs(clients) do    
            c:send({type= "syncroChoix", pck={current="ChoixPerso", player= client.cookie, perso= client.perso}})
        end
    elseif msg.type == "syncro" then
        ...
        if msg.pck.next=="ChoixNiveau" then
            if monde.player1.perso and monde.player2.perso and
                ...
                for k,c in pairs(clients) do
                    c:send({type= "syncro", pck = {current= "ChoixPerso", next = "ChoixNiveau"}})
               end
               ...
               client:send({type= "error", pck={content="Erreur dans le choix des personnages", msg="choix des personnages pas fini"}})
            end
        elseif msg.pck.next == "ChoixTypeJeu" then
            print("GOING BACK SERVER")
            for k,c in pairs(clients) do    
                c.perso = nil
                c:send({type= "syncro", pck = {current= "ChoixPerso", next = "ChoixTypeJeu"}})
            end
