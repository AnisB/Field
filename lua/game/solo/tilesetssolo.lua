...
function TilesetsSolo:getTiles(tiles,map)
	for i,v in pairs(tiles) do
		...
		if v.properties["anim"] == "true" then
			...
			-- 	-- s_resourceManager:newImage(TilesetsSolo.sprites ,map.."-fieldmap/"..v.image, map.."-fieldmap/"..v.image)
			-- 	s_gameStateManager.loader.newImage(TilesetsSolo.sprites ,map.."-fieldmap/"..v.image, map.."-fieldmap/"..v.image)
			-- else
			-- end
			tile.img = s_resourceManager:LoadImage(map.."-fieldmap/"..v.image)
			table.insert(self.tiles,tile)
		end
	end
end
	...
	for i=0,self.layer.height-1,1 do
		for j=1,self.layer.width,1 do
			if self.layer.data[i*self.layer.width+j]>0 then
				...
				if tmp.anim then
					love.graphics.draw(tmp.img:getSprite(),tmp.quad, (j-1)*64-pos.x, (i)*64+pos.y)

				else
					love.graphics.draw(tmp.img,tmp.quad, (j-1)*64-pos.x, (i)*64+pos.y)
				end
			end
		end
	end
